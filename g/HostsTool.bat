...
echo ■-------------------------------------------------------------------■
echo.■   F.修复Hosts文件       P.设置Hosts权限        K.快捷方式修复     ■
echo ■-------------------------------------------------------------------■
echo      G.自动1  U.自动2      X.修复IE  J.工具       T.感谢 H.帮助    
echo ■───────────────────────────────── ■
echo 当前工作目录(O)：%~dp0
echo.
set all=
...
rem Wget下载组件检测
rem Wget下载组件检测
if not exist wget.exe (start /min iexplore http://users.ugent.be/~bpuype/wget/wget.exe)& (echo 正在下载Wget组件，请保存在当前目录！)&pause&goto opdp
title Hosts 数据调整
echo.
echo ○Main Data:                                               
...
cscript //NoLogo /e:vbscript %temp%/Updates.vbs "http://hostsx.googlecode.com/svn/trunk/g/Ver.ini">%temp%/Ver.txt 2>nul
...
:Thanks
echo 特别感谢longsan168对于p处理方面的建议，修改意见，专业上的提醒，以及技术上的支持！
echo 特别感谢sunnyboy8888，vcAngel对于数据方面给我的参考和建议；
echo 特别感谢爽爽亲手绘制的图标~一个卡哇伊的小猴子~ 摸摸~娃哈哈；
echo.
echo --------------------------------------------------------
echo 曾使用过的部分数据或代码来自于：世界之窗论坛，
echo Proxomitron中文站，赢政天下论坛（孤星释狼等），豆瓣小组，
echo 完美者精品论坛（烈火等），霏凡论坛（雪山猎人），
echo Opera论坛（EZibo等）；夜雨十三天，lingshao（迅雷等），
echo pt42 soft，sino blog等，X林的网站屏蔽工具的批处理代码，
echo 部分深山红叶维护光盘里的批处理内容，在此表示感谢！
echo --------------------------------------------------------
pause
echo 感谢未央生，fyi151，lyplay，对于域名转向的建议；
echo 感谢 mickeymouse 对于Hosts的解答；
echo 感谢 bucket，0Cat0 对于fs2u数据的建议；
echo 感谢 hudike，zwy11等对于早期数据的一些误过滤；
echo 感谢 laovo 提供的Hosts修复工具；
echo 感谢 深度诱惑 反馈的百度图片误过滤问题；
echo 感谢 zwy11 对于Blogger，YouTube等GFW相关问题方面的知识普及；
echo 感谢 sebaby 对于数据整理方面和色情网站屏蔽以及数据及时性方面的建议；
echo 感谢 hgldg 等对于色情网站屏蔽方面的建议；
echo 感谢 harveylee 提供的Xmarks的相关数据；
echo 感谢 youth9999 对于土豆网相关问题的建议；
echo 感谢 move 峰旭 等提出的关于自动更新的建议；
echo 感谢 北风其凉 给出的关于UUSEE的建议；
echo 感谢 theshuang 给出的关于搜狗拼音以及银行ssl验误过滤的相关意见；
echo 感谢 laotongbao 给出的增加webgame的相关意见，并感谢其共享的数据；
echo 感谢 fjj0310 sebaby 等对于QQ弹出Hosts修改提示的提醒；
echo 感谢 daewoo223 对于盛大误过滤的反馈；
pause
echo 感谢 xwan008对于灵格斯翻译的相关功能误过滤的提醒；
echo 感谢 wupeng_84对于WMP的过滤提出的相关意见；
echo 感谢 33W66 0Cat0 对于驱动之家过滤后不能显示评论数目的提醒；
echo 感谢 macemo对于P处理执行后显示结果的相关建议；
echo 感谢 夜色茫茫 的不吝夸赞；
echo 感谢 fyi151 sebaby 对于后续版本中的一些小bug的反馈以及相关修改意见；
echo 感谢 汤成翥 给出的关于谷歌拼音同步的问题的建议；
echo 感谢 toshiba 对于百度图片搜索误过滤的反馈；
echo 感谢 YYUU 给出的部分建议和支持；
echo 感谢 mecal 对于不同系统环境下的P处理执行给出的反馈；
echo 感谢 samcsli 对于数据以及其他方面的建议和意见；
echo 感谢 ch4sedream 给出的DropBox的新Ip；
echo 感谢 深度诱惑 反馈的Webqq的登录误过滤问题；
echo 感谢 H2O2 aleax 等对于Xmarks给出的更新建议；
echo 感谢博客上的一些朋友反馈的Vista和Win7的相关问题。
echo 感谢 徐俊 反馈的一些网页的细节问题。
echo --------------------------------------------------------
pause&goto menu

:Help
start /min iexplore http://hi.baidu.com/iebb
mshta vbscript:msgbox("使用中遇到问题请留言，谢谢！",64,"Hosts")(window.close)
echo --------------------------------------------------------
echo File Location:
echo Windows9x: %windir%\hosts
echo Windows32: %windir%\system32\drivers\etc\hosts
echo Windows64: %windir%\system64\drivers\etc\hosts
echo Ubuntu:    sudo gedit /etc/hosts
echo 重启网络：sudo /etc/init.d/networking restart
echo !Ubuntu关闭IPv6
echo 编辑 /etc/modprobe.d/aliases
echo 修改:alias net-pf-10 ipv6 为:alias net-pf-10 off
echo Mac:       sudo niload -v -m hosts
echo 对于广告屏蔽建议使用0.0.0.0，对于验证屏蔽建议使用127.0.0.1
echo --------------------------------------------------------
pause
echo Windows下：
echo HostsTool命令快捷键：
echo  "0"（数字）是打开当前工作目录；
echo  "o"（英文）是打开系统Hosts文件夹；
echo  "g" 是直接在线更新默认数据；
echo  "u" 是直接在线更新缩写版数据；
echo  "c" 是垃圾文件清理；
echo  "e" 是顽固文件（夹）清理；
echo  "a" 是快速添加屏蔽网站；
echo  "w" 查找当前系统Hosts路径，并请自行修改变量；
echo --------------------------------------------------------
echo 常见问题答疑：
echo 1.关于最近一些朋友反馈的脚本错误等问题，
echo 如sopcast等的请使用Pavel的sopcast去广告补丁试试
echo --------------------------------------------------------
echo Acrylic安装使用说明：
echo 在cmd下运行ipconfig /all
echo 1.找到 DNS Servers 后面的那个 IP 地址并记下来，比如是 212.216.112.112;
echo 2.打开 AcrylicConfiguration.ini，把这个 IP 地址填到 PrimaryServerAddress=后面;
echo 3.修改你的本地连接里面的 DNS 地址：在控制面板找到网络连接，选中 TCP/IP，点击属性，勾选使用下面的 DNS 服务器地址，在首选 DNS 服务器里输入 127.0.0.1; 
echo 4.保存之后运行AcrylicTool.bat即可; 
echo --------------------------------------------------------
echo 黑底湖蓝字(03)  黑底白字(07) 红底亮白字(4f)
echo 紫底白字(5f)  亮白底黑字(f0) 绿底白字(2f)                                 
echo 背景 文字 color %a%%b%：0.黑色1.蓝色2.绿色3.浅绿色4.红色 5.紫色
echo 6.黄色7.白色8.灰色9.浅蓝色 A.浅绿B.浅蓝C.浅红D.淡紫E.浅黄F.亮白
echo -------------------------------------------------------------------
pause&goto menu